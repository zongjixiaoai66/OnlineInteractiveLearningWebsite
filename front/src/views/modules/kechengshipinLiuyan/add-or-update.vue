...
<template>
    <div class="addEdit-block">
        <el-form
            ...
            <el-row>
                <el-col :span="12"  v-if="sessionTable !='kechengshipin'">
                    <el-form-item class="select" v-if="type!='info'"  label="课程视频" prop="kechengshipinId">
                        <el-select v-model="ruleForm.kechengshipinId" filterable placeholder="请选择课程视频" @change="kechengshipinChange">
                            <el-option
                                    v-for="(item,index) in kechengshipinOptions"
                                    v-bind:key="item.id"
                                    :label="item.kechengshipinName"
                                    :value="item.id">
                            </el-option>
                        </el-select>
                    </el-form-item>
                ...
                <el-col :span="12"  v-if="sessionTable !='kechengshipin' ">
                    <el-form-item class="input" v-if="type!='info'"  label="视频名称" prop="kechengshipinName">
                        <el-input v-model="kechengshipinForm.kechengshipinName"
                ...
                <el-col :span="24" v-if="sessionTable !='kechengshipin' ">
                    <el-form-item class="upload" v-if="type!='info' && !ro.kechengshipinPhoto" label="视频照片" prop="kechengshipinPhoto">
                        <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in (kechengshipinForm.kechengshipinPhoto || '').split(',')" :src="item" width="100" height="100">
                    </el-form-item>
                    <div v-else>
            ...
            <input id="kechengshipinId" name="kechengshipinId" type="hidden">
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  label="留言内容" prop="kechengshipinLiuyanText">
                        <el-input type="textarea"  :readonly="ro.kechengshipinLiuyanText" v-model="ruleForm.kechengshipinLiuyanText" placeholder="留言内容"></el-input>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.kechengshipinLiuyanText" label="留言内容" prop="kechengshipinLiuyanText">
                            <span v-html="ruleForm.kechengshipinLiuyanText"></span>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
    ...
    export default {
        data() {
            return {
                ...
                yonghuForm: {},
                ro:{
                    kechengshipinId: false,
                    yonghuId: false,
                    kechengshipinLiuyanText: false,
                    replyText: false,
                    insertTime: false,
                    updateTime: false,
                },
                ruleForm: {
                    kechengshipinId: '',
                    yonghuId: '',
                    kechengshipinLiuyanText: '',
                    replyText: '',
                    insertTime: '',
                    updateTime: '',
                },
                kechengshipinOptions : [],
                yonghuOptions : [],
                rules: {
                   kechengshipinId: [
        ...
        created() {
            ...
            //当前text不能修改
            this.ro.kechengshipinLiuyanText = true;
            if (this.role != "管理员"){
            }
         ...
         this.$http({
             url: `kechengshipin/page?page=1&limit=100`,
             method: "get"
         }).then(({ data }) => {
             if (data && data.code === 0) {
                this.kechengshipinOptions = data.data.list;
            }
         });
         this.$http({
        ...
        methods: {
            ...
            init(id,type) {
                ...
                if(this.type=='info'||this.type=='else'){
                    ...
                    for (var o in obj){
                      ...
                      if(o=='kechengshipinId'){
                          this.ruleForm.kechengshipinId = obj[o];
                          this.ro.kechengshipinId = true;
                          continue;
                      }
                      if(o=='yonghuId'){
                      ...
                      if(o=='kechengshipinLiuyanText'){
                          this.ruleForm.kechengshipinLiuyanText = obj[o];
                          this.ro.kechengshipinLiuyanText = true;
                          continue;
                      }
                      if(o=='replyText'){
            ...
            kechengshipinChange(id){
                this.$http({
                    url: `kechengshipin/info/`+id,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.kechengshipinForm = data.data;
                    }
                });
            },
            yonghuChange(id){
            ...
            info(id) {
                this.$http({
                    url: `kechengshipinLiuyan/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.ruleForm = data.data;
                        this.kechengshipinChange(data.data.kechengshipinId)
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`kechengshipinLiuyan/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.kechengshipinLiuyanCrossAddOrUpdateFlag = false;
                                        this.parent.search();
